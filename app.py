...
def load_data():
    # Use the absolute path to the tarji directory
    outputs_dir = "c:\\Users\\eyabe\\OneDrive\\Bureau\\2LNIG\\sem2\\PROBSOLV\\SFWRE-G4\\amine\\data\\tarji"

    # Debug: List all files in the directory
    print(f"Directory contents of {os.path.abspath(outputs_dir)}:")
    ...
    # Check files starting with summary_{scenario}_cap{capacity}_day{day}
    summary_files = [f"summary_{scenario}_cap{capacity}_day{day}" for scenario in scenarios for capacity in capacities for day in days]
    existing_summary_files = []
    summary_dfs = []
    
    # Debug: Check if files exist
    print(f"Looking for summary files in directory: {os.path.abspath(outputs_dir)}")
    for file_prefix in summary_files:
        for file in os.listdir(outputs_dir):
            if file.startswith(file_prefix) and file.endswith(".csv"):
                file_path = os.path.join(outputs_dir, file)
                existing_summary_files.append(file)
                break  # Take the first match for now, adjust if multiple timestamps exist
    ...
    for file in existing_summary_files:
        try:
            ...
            parts = file.split("_")
            scenario = parts[1]
            capacity = int(parts[2].replace("cap", ""))
            day = int(parts[3].replace("day", "").split("_")[0])
    ...
    if summary_dfs:
        ...
        # Check files starting with selected_items_cap{capacity}_day{day}_{date} with date 1 to 30
        allocated_files = [f"selected_items_cap{capacity}_day{day}_{date}" for capacity in capacities for day in days for date in range(1, 31)]
        ...
        for file_prefix in allocated_files:
            for file in os.listdir(outputs_dir):
                if file.startswith(file_prefix.replace("_", "", 1).replace("day", "")) and file.endswith(".csv"):  # Adjust for date format
                    file_path = os.path.join(outputs_dir, file)
        ...
        for file in existing_allocated_files:
            try:
                ...
                parts = file.split("_")
                capacity = int(parts[1].replace("cap", ""))
                day = int(parts[2].replace("day", "").split("_")[0])
                df["scenario"] = parts[0].replace("selecteditems", "urban")  # Assuming scenario from prefix
                df["capacity"] = capacity
                df["day"] = day
        ...
        if allocated_dfs:
            ...
            print("No summary or allocated items files found in amine\\data\\tarji directory. Using fallback data.")
            df = pd.DataFrame({
